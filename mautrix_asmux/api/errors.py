...
class _ErrorMeta:
    ...
    def request_not_json(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_NOT_JSON", "Request body is not valid JSON")
        )

    @property
    def missing_appservice_id_query(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_MISSING_PARAM", "Missing appservice_id query param")
        )

    @property
    def invalid_appservice_id_query(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_INVALID_PARAM", "Malformed appservice_id query param")
        )

    @property
    def missing_auth_header(self) -> web.HTTPException:
        return web.HTTPForbidden(
            **self._make_error("M_MISSING_TOKEN", "Missing authorization header")
        )

    @property
    def invalid_auth_header(self) -> web.HTTPException:
        return web.HTTPForbidden(
            **self._make_error("M_UNKNOWN_TOKEN", "Invalid authorization header")
        )

    @property
    def invalid_auth_token(self) -> web.HTTPException:
        return web.HTTPForbidden(
            **self._make_error("M_UNKNOWN_TOKEN", "Invalid authorization token")
        )

    @property
    def invalid_config_password(self) -> web.HTTPException:
        return web.HTTPForbidden(
            **self._make_error("M_UNKNOWN_TOKEN", "Invalid config download password")
        )
    ...
    def room_delete_access_denied(self) -> web.HTTPException:
        return web.HTTPUnauthorized(
            **self._make_error("M_UNAUTHORIZED", "You are not authorized to delete rooms")
        )
    ...
    def user_access_denied(self) -> web.HTTPException:
        return web.HTTPUnauthorized(
            **self._make_error("M_UNAUTHORIZED", "You are not authorized to access that user")
        )
    ...
    def invalid_owner(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_BAD_REQUEST", "Appservice owner must match [a-z0-9=.-]{1,32}")
        )

    @property
    def invalid_prefix(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_BAD_REQUEST", "Appservice prefix must match [a-z0-9=.-]{1,32}")
        )

    @property
    def invalid_uuid(self) -> web.HTTPException:
        return web.HTTPBadRequest(**self._make_error("M_BAD_REQUEST", "Invalid UUID"))

    @property
    def missing_fields(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_BAD_REQUEST", "Missing one or more fields in request")
        )

    @property
    def bridge_query_required(self) -> web.HTTPException:
        return web.HTTPBadRequest(
            **self._make_error("M_BAD_REQUEST", "`bridge` query param is required")
        )
    ...
    def appservice_not_found(self) -> web.HTTPException:
        return web.HTTPNotFound(
            **self._make_error("M_NOT_FOUND", "Application service not found.")
        )

    @property
    def user_not_found(self) -> web.HTTPException:
        return web.HTTPNotFound(**self._make_error("M_NOT_FOUND", "User not found."))

    @property
    def proxy_not_setup(self) -> web.HTTPException:
        return web.HTTPNotFound(**self._make_error("M_NOT_FOUND", "Proxy config not set."))

    @property
    def failed_to_contact_homeserver(self) -> web.HTTPException:
        return web.HTTPBadGateway(
            **self._make_error("M_BAD_GATEWAY", "Failed to contact homeserver")
        )

    @property
    def failed_to_register_bot(self) -> web.HTTPException:
        return web.HTTPInternalServerError(
            **self._make_error("M_UNKNOWN", "Failed to register bridge bot")
        )

    @property
    def websocket_not_connected(self) -> web.HTTPException:
        raise web.HTTPBadGateway(
            **self._make_error("FI.MAU.WS_NOT_CONNECTED", "Endpoint is not connected to websocket")
        )
